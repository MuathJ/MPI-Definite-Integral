...
// mpic++ -o mpi_integrate_v3 mpi_integrate_v3.cpp
...
int main(int argc, char **argv) 
{
	...
	int q = my_rank/4;
    MPI_Comm NEW_COMM;
    MPI_Comm_split(MPI_COMM_WORLD, q, my_rank, &NEW_COMM);
	...
	if (new_rank != 0) 
	{	MPI_Send (&local_int, 1, MPI_DOUBLE, 0, 0, NEW_COMM);
    }
    
    else 
    {	total_int = local_int;
		for (source=1 ; source<new_size ; source++) 
			{	MPI_Recv(&local_int, 1, MPI_DOUBLE, source, 0, NEW_COMM, MPI_STATUSES_IGNORE);
				total_int += local_int;
			}
    }
